...
@Component({
  ...
  templateUrl: './approvereject.page.html',
  styleUrls: ['./approvereject.page.scss'],
})
    ...
    if(this.rejected_reason==""){
      ...
      this.acessPr.postreason(body).subscribe((res:any)=>{
        if(res.status==true){
         ...
         this.http.get(AccessProviders.server+'/dltapprove/'+this.application_id).map(res=>res).subscribe((res:any)=>{
